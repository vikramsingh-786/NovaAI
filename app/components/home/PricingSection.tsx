...
export default function PricingSection() {
  ...
  const handleUpgrade = async () => {
    if (!isSignedIn) {
      ...
      const redirectParam = `redirect_url=${encodeURIComponent(
        window.location.pathname + "?action=upgrade"
      )}`;
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          ...
          .catch(() => ({ error: "Failed to process request." }));
        throw new Error(
          errorData.error || "Failed to open subscription portal"
        );
      }
    ...
    <section className="py-16 px-4 sm:px-6 lg:px-8" id="pricing">
      ...
      <div className="max-w-md mx-auto bg-[var(--card-bg)] rounded-xl shadow-2xl p-8 border border-[var(--border-primary)]">
        ...
        {isSubscriptionLoading ? (
          <button
            disabled
            className="w-full px-8 py-3.5 rounded-lg bg-gray-500 opacity-70 text-white flex justify-center items-center text-lg font-semibold"
          >
          ...
          >
            {isCheckoutLoading ? (
              ...
              <>
                {isSignedIn ? "Upgrade to Pro" : "Get Started with Pro"}{" "}
        ...
        {subscriptionStatus &&
          ...
          !isSubscriptionLoading && (
            <p className="text-center mt-4 text-sm text-[var(--text-muted)]">
              Your current plan:{" "}
              <span className="font-semibold capitalize text-[var(--accent-purple)]">
